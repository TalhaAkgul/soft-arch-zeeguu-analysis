        ...
        AssignmentExpression(path) {
            ...
            if (
                ...
                if (node.right.type === "FunctionExpression" || node.right.type === "ArrowFunctionExpression") {
                    // Save this as the current function context on this node
                    path.setData("functionName", methodName);
                    // Also store for use by call expressions within this body
                    path.get("right").traverse({
                        CallExpression(callPath) {
                            const callee = callPath.node.callee;
                            const functionName = methodName;
                            const endpointArg = callPath.node.arguments[0];
                            const endpoint = resolveEndpoint(endpointArg, variableMap);
                            ...
                            if (
                                ...
                                ["_getJSON", "_getPlainText", "_post", "apiGet", "apiPost"].includes(callee.property.name)
                            ) {
            ...
            // existing variable tracking (keep this part)
            if (node.left.type === "Identifier" && node.right) {
                if (
                    node.right.type === "BinaryExpression" &&
                    (node.right.operator === "+" || node.right.operator === "+=")
                ) {
                    let value = resolveConcatenation(node.right);
                    if (value) variableMap.set(node.left.name, { type: "StringLiteral", value });
                } else {
